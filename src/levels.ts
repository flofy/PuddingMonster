import { LevelData, Pudding, Star, WorldData } from './types';
import { v4 as uuidv4 } from 'uuid';

const createPudding = (x: number, y: number, type: any, id: string): Pudding => ({
  id,
  type,
  x,
  y,
...
const generateLevels = (): WorldData[] => {
  ...
  const worldThemes = ['forest', 'desert', 'ice', 'volcano', 'space'];
  ...
  for (let worldIdx = 0; worldIdx < worldThemes.length; worldIdx++) {
    ...
    for (let levelIdx = 0; levelIdx < 25; levelIdx++) {
      ...
      let puddingTypes: any[];

      switch (levelIdx % 4) {
        case 0:
          positions = gridPositions(100, 100, 50, Math.ceil(puddingCount / 3), 3);
          puddingTypes = positions.map((_, i) => ['normal', 'green', 'purple'][(i + levelIdx) % 3]);
          break;
        case 1:
          positions = circlePositions(250, 250, 80 + levelIdx * 5, puddingCount);
          puddingTypes = positions.map((_, i) => ['normal', 'green', 'purple', 'normal'][(i + levelIdx) % 4]);
          break;
        case 2:
      ...
      const puddings = positions.slice(0, puddingCount).map((pos, i) =>
        createPudding(pos.x, pos.y, puddingTypes[i], uuidv4())
      );
    ...
    worlds.push({
      name: worldNames[worldIdx],
      theme: worldThemes[worldIdx] as any,
      levels,
    });
  }
...
export const getTotalLevels = (): number => {
  const worlds = getWorlds();
  return worlds.reduce((total, world) => total + world.levels.length, 0);
};
